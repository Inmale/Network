...
private:
	...
	int m_sum{ 0 };
public:
	SumHandler(node_ptr sender, size_t recipient_id) : m_sender(sender), m_recipient_id(recipient_id) {};
...
class CountHandler : public IEventHandler
{
	...
	int m_count{ 0 };
public:
	CountHandler(node_ptr sender, size_t recipient_id) : m_sender(sender), m_recipient_id(recipient_id) {};
